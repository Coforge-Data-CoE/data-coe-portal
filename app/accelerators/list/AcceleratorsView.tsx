      ...
      {view === "table" && (
        <Table
          dataSource={items.map((it: any) => ({
            ...
            image: it.iconUrl ? <img src={it.iconUrl} alt={it.name} style={{ maxWidth: 80, borderRadius: 6 }} /> : null,
            video: it.videoUrl ? <video src={it.videoUrl} controls style={{ maxWidth: 120, borderRadius: 6 }} /> : null,
            actions: (
              <div className="flex gap-2">
                <Link href={`../accelerators/${it._id}`} className="px-3 py-1 rounded bg-blue-500 text-white text-xs font-semibold hover:bg-blue-600 transition">View</Link>
                <Link href={`../accelerators/new?id=${it._id}`} className="px-3 py-1 rounded bg-orange-500 text-white text-xs font-semibold hover:bg-orange-600 transition">Edit</Link>
              </div>
            ),
          }))}
          columns={[
        ...
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-5">
          {items.map((it: any) => (
            <div key={it._id} className="bg-white rounded-xl shadow-lg flex flex-col overflow-hidden">
              {it.iconUrl ? (
                <img src={it.iconUrl} alt={it.name} className="w-full h-44 object-contain" />
              ) : (
        ...
        <ul style={{ listStyle: "none", padding: 0 }}>
          {items.map((it: any) => (
            <li key={it._id} style={{ margin: "18px 0", background: "#fff", borderRadius: 8, padding: 16, color: "#222", boxShadow: "0 2px 12px #0001" }}>
              ...
              {it.iconUrl ? (
                <div>
                  <img src={it.iconUrl} alt={it.name} style={{ maxWidth: 120, borderRadius: 6, marginTop: 8 }} />
                </div>
              ) : null}
              {it.videoUrl ? (
