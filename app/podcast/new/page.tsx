...
import { PodcastCard, Podcast as UIPodcast } from "@/app/components/PodcastCard";
...
const NewPodcastPage = () => {
  ...
  useEffect(() => {
    ...
    async function loadById() {
      try {
        setEditLoading(true);
        setEditError(null);
        const res = await fetch(apiUrl(`/api/podcasts/${id}`), { cache: "no-store" });
        if (!res.ok) {
  ...
  async function onSubmit(e: React.FormEvent, releaseFlag: boolean) {
    ...
    try {
      ...
      else if (selectedDraft?.audioUrl) fd.append("podcast_url", selectedDraft.audioUrl);
      if (coverFile) fd.append("cover", coverFile, coverFile.name);
      else if (selectedDraft?.coverImage) fd.append("cover_image", selectedDraft.coverImage);

      const url = selectedDraft ? apiUrl(`/api/podcasts/${selectedDraft.id}`) : apiUrl("/api/podcasts");
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="mx-auto max-w-6xl grid grid-cols-12 gap-6">
        ...
        {editLoading && (
          <div className="mb-3 text-sm text-gray-600">Loading podcast for editing…</div>
        )}
        {editError && (
        ...
        <form onSubmit={(e) => onSubmit(e, false)} className="space-y-4">
          ...
          <div>
            ...
            {selectedDraft?.audioUrl && !audioFile && (
              <div className="mt-2 flex items-center gap-2 text-xs text-gray-600">
                <span className="inline-block px-2 py-1 bg-gray-100 rounded">Using draft audio</span>
                <a href={selectedDraft.audioUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">Open</a>
              </div>
            )}
          </div>
          <div>
            ...
            {selectedDraft?.coverImage && !coverFile && (
              <div className="mt-2 flex items-center gap-2 text-xs text-gray-600">
                <span className="inline-block px-2 py-1 bg-gray-100 rounded">Using draft cover</span>
                <a href={selectedDraft.coverImage} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">Open</a>
              </div>
            )}
          </div>
          <div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              title="Save as draft"
            >
              {submitting ? (selectedDraft ? "Updating…" : "Saving…") : (selectedDraft ? "Update Draft" : "Save Draft")}
            </button>
            <button
              type="button"
              disabled={submitting}
              ...
              title="Release now"
            >
              {submitting ? "Releasing…" : (selectedDraft ? "Release Changes" : "Release")}
            </button>
            <button
              type="button"
              onClick={doPreview}
                ...
                {drafts.map((d) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-lg border hover:bg-gray-50 ${
                      selectedDraft?.id === d.id ? "border-blue-500" : "border-gray-200"
                    }`}
                  >
                    <img
                      src={d.coverImage || d.authorAvatar || "https://placehold.co/48x48"}
                      alt={d.title}
                      className="w-12 h-12 rounded-lg object-cover"
                    />
                    <div className="text-left">
                      <div className="text-sm font-medium text-gray-800 line-clamp-1">{d.title}</div>
                      <div className="text-xs text-gray-500 line-clamp-1">{d.author}</div>
                    </div>
                  </button>
                ))}
              </div>
