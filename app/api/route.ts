  ...
  const summary = (form.get('summary') as string)?.trim() || ''
    ...
    if (iconFile && iconFile.size > 0) {
      try {
        ...
        const { url } = await uploadBufferToBlob('accelerator-icons', buffer, iconFile.type, iconFile.name)
        iconUrl = url
      } catch (e: any) {
    ...
    if (bannerFile && bannerFile.size > 0) {
      try {
        ...
        const { url } = await uploadBufferToBlob('accelerator-banners', buffer, bannerFile.type, bannerFile.name)
        imageUrl = url
      } catch (e: any) {
    ...
    if (videoFile && videoFile.size > 0) {
      try {
        ...
        const { url } = await uploadBufferToBlob('accelerator-videos', buffer, videoFile.type, videoFile.name)
        videoUrl = url
      } catch (e: any) {
